...
namespace Detour
{
	...
	public class NavMeshQuery
	{
        ...
        private static long[] m_polys = new long[128];
        private static long[] m_tmpPolys = new long[256];
        private static float[] m_closestPoint = new float[3];
        private static float[] m_verts = new float[3 * NavMeshBuilder.VertsPerPoly];
        private static float[] m_edged = new float[NavMeshBuilder.VertsPerPoly];
        private static float[] m_edget = new float[NavMeshBuilder.VertsPerPoly];
        private static float[] m_areas = new float[NavMeshBuilder.VertsPerPoly];
        private static float[] m_pos = new float[3];
        private static MeshTile[] m_neis = new MeshTile[32];
        private static int[] m_nbmin = new int[3];
        private static int[] m_nbmax = new int[3];
        private static float[] m_fbmin = new float[3];
        private static float[] m_fbmax = new float[3];
        private static float[] m_fbmin2 = new float[3];
        private static float[] m_fbmax2 = new float[3];

        private static float[] m_v = new float[9];
        private static float[] m_left = new float[3];
        private static float[] m_right = new float[3];
        private static float[] m_centerPos = new float[3];
            ...
            while (!_openList.Empty())
            {
                ...
                for (long i = bestPoly.FirstLink; i != NavMesh.NullLink; i = bestTile.Links[i].Next)
                {
                    ...
                    if (bestTile.Links[i].Side != 0xff)
                        crossSide = (short)(bestTile.Links[i].Side >> 1);

                    Node neighbourNode = NodePool.GetNode(neighborRef, crossSide);
                    ...
                    if (neighbourNode.Flags == 0)
                    {
                        ...
                        GetEdgeMidPoint(bestRef, bestPoly, bestTile, neighborRef, neighbourPoly, neighbourTile, ref m_pos);
                        Array.Copy(m_pos, neighbourNode.Pos, 3);
                    }
	                                   ...
	                                   ref int straightPathCount, int maxStraightPath, int options = 0)
	    {
            ...
            if (pathSize > 1)
            {
                ...
                for (int i = 0; i < pathSize; i++)
                {
                    ...
                    Array.Clear(m_left, 0, m_left.Length);
                    Array.Clear(m_right, 0, m_right.Length);

                    short fromType = 0, toType = 0;

                    if (i + 1 < pathSize)
                    {
                        if ((GetPortalPoints(path[i], path[i + 1], ref m_left, ref m_right, ref fromType, ref toType) &
                             Status.Failure) != 0)
                        {
                        ...
                        if (i == 0)
                        {
                            float t;
                            if (
                                Helper.DistancePtSegSqr2D(portalApex[0], portalApex[1], portalApex[2], m_left[0], m_left[1],
                                                          m_left[2], m_right[0], m_right[1], m_right[2], out t) <
                                0.001f*0.001f)
                                continue;
                        }
                    }
                    ...
                    {
                        Array.Copy(closestEndPos, m_left, 3);
                        Array.Copy(closestEndPos, m_right, 3);

                        fromType = toType = NavMeshBuilder.PolyTypeGround;
                    }

                    if (Helper.TriArea2D(portalApex, portalRight, m_right) <= 0.0f)
                    {
                        if (Helper.VEqual(portalApex[0], portalApex[1], portalApex[2], portalRight[0], portalRight[1], portalRight[2]) ||
                            Helper.TriArea2D(portalApex, portalLeft, m_right) > 0.0f)
                        {
                            Array.Copy(m_right, portalRight, 3);
                    ...
                    if (Helper.TriArea2D(portalApex, portalLeft, m_left) >= 0.0f)
                    {
                        if (Helper.VEqual(portalApex[0], portalApex[1], portalApex[2], portalLeft[0], portalLeft[1],
                                          portalLeft[2]) || Helper.TriArea2D(portalApex, portalRight, m_left) < 0.0f)
                        {
                            Array.Copy(m_left, portalLeft, 3);
                ...
                for (long i = bestPoly.FirstLink; i != NavMesh.NullLink; i = bestTile.Links[i].Next)
                {
                    ...
                    if (neighborNode.Flags == 0)
                    {
                        ...
                        GetEdgeMidPoint(bestRef, bestPoly, bestTile, neighborRef, neighborPoly, neighborTile,
                                        ref m_pos);
                        Array.Copy(m_pos, neighborNode.Pos, 3);
                    }
        ...
        public Status FinalizeSlicedFindPathPartial(long[] existing, int existingSize, ref long[] path, ref int pathCount,
                                                    int maxPath)
        {
            pathCount = 0;
            if(existingSize == 0)
            ...
            while (!_openList.Empty())
            {
                ...
                for (long i = bestPoly.FirstLink; i != NavMesh.NullLink; i = bestTile.Links[i].Next)
                {
                    ...
                    if (neighborNode.Flags == 0)
                    {
                        ...
                        Helper.VLerp(ref m_pos, va[0], va[1], va[2], vb[0], vb[1], vb[2], 0.5f);
                        Array.Copy(m_pos, neighborNode.Pos, 3);
                    }
                                           ...
                                           ref int resultCount, int maxResult)
        {
            ...
            Array.Clear(m_centerPos, 0, m_centerPos.Length);
            for (int i = 0; i < nverts; i++)
            {
                Helper.VAddEx(m_centerPos[0], m_centerPos[1], m_centerPos[2], verts[i * 3 + 0], verts[i * 3 + 1], verts[i * 3 + 2], ref m_centerPos);
            }

            m_centerPos = Helper.VScale(m_centerPos[0], m_centerPos[1], m_centerPos[2], 1.0f / nverts);

            Node startNode = NodePool.GetNode(startRef);
            Array.Copy(m_centerPos, startNode.Pos, 3);
            startNode.PIdx = 0;
            startNode.Cost = 0;
            startNode.Total = 0;
            startNode.Id = startRef;
                                      ...
                                      ref float[] nearestPt)
        {
            ...
            int polyCount = 0;
            if ((QueryPolygons(center, extents, filter, ref m_polys, ref polyCount, m_polys.Length) & Status.Failure) != 0)
                return Status.Failure | Status.InvalidParam;
            ...
            {
                long refId = m_polys[i];
                ClosestPointOnPoly(refId, center, ref m_closestPoint);
                float d = Helper.VDistSqr(center[0], center[1], center[2], m_closestPoint[0], m_closestPoint[1],
                                          m_closestPoint[2]);
                if (d < nearestDistanceSqr)
                {
                    if (nearestPt != null)
                    {
                        Array.Copy(m_closestPoint, nearestPt, 3);
                    }
        ...
        // add by jiangzhiqiang1@kingsoft.com at 2015.10.19 for 2D nearest poly check
        public Status FindNearestPoly2D(float[] center, float[] extents, QueryFilter filter, ref long nearestRef, ref float[] nearestPt) {
            if (NavMesh == null)
                throw new ApplicationException("NavMesh is not initialized");
            ...
            for (int i = 0; i < polyCount; i++) {
                ...
                float d = Helper.VDistSqr(center[0], 0.0f, center[2], m_closestPoint[0], 0.0f,
                                          m_closestPoint[2]);

                if (d < nearestDistanceSqr ||
                    (d == nearestDistanceSqr && maxHeight < m_closestPoint[1])) {
                    if (nearestPt != null) {
                        Array.Copy(m_closestPoint, nearestPt, 3);
                        maxHeight = m_closestPoint[1];
                    }
                    nearestDistanceSqr = d;
                    nearest = refId;
                }
                                    ...
                                    ref int polyCount, int maxPolys)
        {
            ...
            Helper.VSubEx(center[0], center[1], center[2], extents[0], extents[1], extents[2], ref m_fbmin2);
            Helper.VAddEx(center[0], center[1], center[2], extents[0], extents[1], extents[2], ref m_fbmax2);

            int minx, miny, maxx, maxy;
            NavMesh.CalcTileLoc(m_fbmin2[0], m_fbmin2[1], m_fbmin2[2], out minx, out miny);
            NavMesh.CalcTileLoc(m_fbmax2[0], m_fbmax2[1], m_fbmax2[2], out maxx, out maxy);

            //int MaxNeis = 32;
            //MeshTile[] neis = new MeshTile[MaxNeis];
            Array.Clear(m_neis, 0, m_neis.Length);

            long[] tempPolys = m_tmpPolys;
            if (tempPolys.Length < maxPolys) {
                tempPolys = new long[maxPolys];
            }
            ...
            for (int y = miny; y <= maxy; y++)
            {
                ...
                {
                    int nneis = NavMesh.GetTilesAt(x, y, ref m_neis, m_neis.Length);
                    for (int j = 0; j < nneis; j++)
                    {
                        //long[] tempPolys = new long[maxPolys-n];
                        int tempn = QueryPolygonsInTile(m_neis[j], m_fbmin2, m_fbmax2, filter, ref tempPolys, maxPolys - n);
                              ...
                              ref float[] hitNormal, ref long[] path, ref int pathCount, int maxPath)
        {
            ...
            int n = 0;
            Array.Clear(m_verts, 0, m_verts.Length);
            ...
            while (curRef > 0)
            {
                ...
                {
                    Array.Copy(tile.Verts, poly.Verts[i] * 3, m_verts, nv * 3, 3);
                    nv++;
                }

                float tmin, tmax;
                int segMin, segMax;
                if (!Helper.IntersectSegmentPoly2D(startPos, endPos, m_verts, nv, out tmin, out tmax, out segMin,
                                                  out segMax))
                {
                    pathCount = n;
                    return status;
                ...
                if (nextRef == 0)
                {
                    ...
                    float dx = m_verts[b * 3 + 0] - m_verts[a * 3 + 0];
                    float dz = m_verts[b * 3 + 2] - m_verts[a * 3 + 2];
                    hitNormal[0] = dz;
                    hitNormal[1] = 0;
                    hitNormal[2] = -dx;
        ...
        public Status FindRandomPoint(QueryFilter filter, RandFunc func, ref long randomRef, ref float[] randomPt)
        {
            ...
            Array.Clear(m_verts, 0, m_verts.Length);
            Array.Clear(m_areas, 0, m_areas.Length);

            Array.Copy(tile.Verts, v, m_verts, 0, 3);
            for (int j = 1; j < poly.VertCount; j++)
            {
                v = poly.Verts[j]*3;
                Array.Copy(tile.Verts, v, m_verts, j * 3, 3);
            }
            ...
            Helper.RandomPointInConvexPoly(m_verts, poly.VertCount, ref m_areas, s, t, ref pt);
                                                  ...
                                                  RandFunc frand, ref long randomRef, ref float[] randomPt)
        {
            ...
            Array.Copy(randomTile.Verts, v, m_verts, 0, 3);
            for (int j = 1; j < randomPoly.VertCount; j++)
            {
                v = randomPoly.Verts[j]*3;
                Array.Copy(randomTile.Verts, v, m_verts, j * 3, 3);
            }
            ...
            float[] pt = new float[3];
            Helper.RandomPointInConvexPoly(m_verts, randomPoly.VertCount, ref m_areas, s, t, ref pt);
        ...
        public Status ClosestPointOnPolyBoundary(long refId, float[] pos, ref float[] closest)
        {
            ...
            Array.Clear(m_verts, 0, m_verts.Length);
            Array.Clear(m_edged, 0, m_edged.Length);
            Array.Clear(m_edget, 0, m_edget.Length);
            ...
            bool inside = Helper.DistancePtPolyEdgesSqr(pos[0], pos[1], pos[2], m_verts, nv, ref m_edged, ref m_edget);
            if (inside)
            {
            ...
            else
            {
                ...
                int va = imin*3;
                int vb = ((imin + 1)%nv)*3;
                Helper.VLerp(ref closest, m_verts[va + 0], m_verts[va + 1], m_verts[va + 2], m_verts[vb + 0], m_verts[vb + 1], m_verts[vb + 2], m_edget[imin]);
            }
            return Status.Success;
        }
            ...
            else
            {
                ...
                for (int j = 0; j < pd.TriCount; j++)
                {
                    ...
                    for (int k = 0; k < 3; k++)
                    {
                        ...
                        {
                            Array.Copy(tile.Verts, poly.Verts[tile.DetailTris[t + k]] * 3, m_v, k*3, 3);
                        }
                        else
                        {
                            Array.Copy(tile.DetailVerts, (pd.VertBase + (tile.DetailTris[t + k] - poly.VertCount)) * 3, m_v, k * 3, 3);                            
                        }
                    }
                    float h = 0.0f;
                    if (Helper.ClosestHeightPointTriangle(pos[0], pos[1], pos[2], m_v[0], m_v[1], m_v[2], m_v[3], m_v[4], m_v[5], m_v[6], m_v[7], m_v[8], ref h))
                    {
                        height = h;
                        return Status.Success;
                    }
                                        ...
                                        int maxPolys)
        {
            ...
            if (tile.BVTree != null)
            {
                ...
                Array.Clear(m_nbmax, 0, m_nbmax.Length);
                Array.Clear(m_nbmin, 0, m_nbmin.Length);
                ...
                m_nbmin[0] = (int)(qfac * minx) & 0xfffe;
                m_nbmin[1] = (int)(qfac * miny) & 0xfffe;
                m_nbmin[2] = (int)(qfac * minz) & 0xfffe;
                m_nbmax[0] = (int)(qfac * maxx + 1) | 1;
                m_nbmax[1] = (int)(qfac * maxy + 1) | 1;
                m_nbmax[2] = (int)(qfac * maxz + 1) | 1;

                long baseRef = NavMesh.GetPolyRefBase(tile);
                int n = 0;
                while (node < end)
                {
                    bool overlap = Helper.OverlapQuantBounds(m_nbmin, m_nbmax, tile.BVTree[node].BMin, tile.BVTree[node].BMax);
            ...
            else
            {
                ...
                Array.Clear(m_fbmax, 0, m_fbmax.Length);
                Array.Clear(m_fbmin, 0, m_fbmin.Length);
                ...
                for (int i = 0; i < tile.Header.PolyCount; i++)
                {
                    ...
                    int v = p.Verts[0]*3;
                    Array.Copy(tile.Verts, v, m_fbmin, 0, 3);
                    Array.Copy(tile.Verts, v, m_fbmax, 0, 3);
                    for (int j = 1; j < p.VertCount; j++)
                    {
                        v = p.Verts[j]*3;
                        Helper.VMin(ref m_fbmin, tile.Verts[v + 0], tile.Verts[v + 1], tile.Verts[v + 2]);
                        Helper.VMax(ref m_fbmax, tile.Verts[v + 0], tile.Verts[v + 1], tile.Verts[v + 2]);
                    }
                    if (Helper.OverlapBounds(qmin[0], qmin[1], qmin[2], qmax[0], qmax[1], qmax[2], m_fbmin[0], m_fbmin[1],
                                             m_fbmin[2], m_fbmax[0], m_fbmax[1], m_fbmax[2]))
                    {
                        if (n < maxPolys)
                            polys[n++] = refId;
                    }
                                          ...
                                          ref float[] nearestPt)
        {
            ...
            Helper.VSubEx(center[0], center[1], center[2], extents[0], extents[1], extents[2], ref m_fbmin);
            Helper.VAddEx(center[0], center[1], center[2], extents[0], extents[1], extents[2], ref m_fbmax);

            int polyCount = QueryPolygonsInTile(tile, m_fbmin, m_fbmax, filter, ref m_polys, m_polys.Length);
            ...
            for (int i = 0; i < polyCount; i++)
            {
                ...
                ClosestPointOnPolyInTile(tile, poly, center, ref m_closestPoint);

                float d = Helper.VDistSqr(center[0], center[1], center[2], m_closestPoint[0], m_closestPoint[1], m_closestPoint[2]);
                if (d < nearestDistanceSqr)
                {
                    Array.Copy(m_closestPoint, nearestPt, 3);
                    nearestDistanceSqr = d;
        ...
        private void ClosestPointOnPolyInTile(MeshTile tile, Poly poly, float[] pos, ref float[] closest)
        {
            ...
            for (int i = 0; i < nv; i++)
            {
                Array.Copy(tile.Verts, poly.Verts[i] * 3, m_verts, i * 3, 3);
            }

            Array.Copy(pos, closest, 3);
            if (!Helper.DistancePtPolyEdgesSqr(pos[0], pos[1], pos[2], m_verts, nv, ref m_edged, ref m_edget))
            {
                float dmin = float.MaxValue;
                int imin = -1;
                for (int i = 0; i < nv; i++)
        ...
        private Status GetEdgeMidPoint(long from, long to, ref float[] mid)
        {
            ...
            short fromType = 0, toType = 0;
            if ((GetPortalPoints(from, to, ref m_left, ref m_right, ref fromType, ref toType) & Status.Failure) != 0)
            {
                return Status.Failure |  Status.InvalidParam;
            }
            mid[0] = (m_left[0] + m_right[0]) * 0.5f;
            mid[1] = (m_left[1] + m_right[1]) * 0.5f;
            mid[2] = (m_left[2] + m_right[2]) * 0.5f;
            return Status.Success;
        }
                                      ...
                                      ref float[] mid)
        {
            ...
            short fromType = 0, toType = 0;
            if ((GetPortalPoints(from, fromPoly, fromTile, to, toPoly, toTile, ref m_left, ref m_right) & Status.Failure) != 0)
            {
                return Status.Failure | Status.InvalidParam;
            }
            mid[0] = (m_left[0] + m_right[0]) * 0.5f;
                                     ...
                                     int maxStraightPath, int options)
        {
            ...
            for (int i = startIdx; i < endIdx; i++)
            {
                ...
                if (Helper.IntersectSegSeg2D(straightPath[startPos + 0], straightPath[startPos + 1],
                                             straightPath[startPos + 2], endPos[0], endPos[1], endPos[2], m_left, m_right,
                                             ref s, ref t))
                {
                    float[] pt = new float[3];
                    Helper.VLerp(ref pt, m_left[0], m_left[1], m_left[2], m_right[0], m_right[1], m_right[2], t);
