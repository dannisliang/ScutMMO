...
namespace Recast.Data
{
    public class Helper
    {
        ...
        public static float [] m_v0 = new float[3];
        public static float [] m_v1 = new float[3];
        public static float [] m_v2 = new float[3];
        ...
        {
            VSubEx(cx, cy, cz, ax, ay, az, ref m_v0);
            VSubEx(bx, by, bz, ax, ay, az, ref m_v1);
            VSubEx(px, py, pz, ax, ay, az, ref m_v2);

            float dot00 = VDot2D(m_v0, m_v0);
            float dot01 = VDot2D(m_v0, m_v1);
            float dot02 = VDot2D(m_v0, m_v2);
            float dot11 = VDot2D(m_v1, m_v1);
            float dot12 = VDot2D(m_v1, m_v2);
        ...
        public static void VSubEx(float v1x, float v1y, float v1z, float v2x, float v2y, float v2z, ref float[] dest) {
            dest[0] = v1x - v2x;
            dest[1] = v1y - v2y;
            dest[2] = v1z - v2z;
        }

        public static void VAddEx(float v1x, float v1y, float v1z, float v2x, float v2y, float v2z, ref float[] dest) {
            dest[0] = v1x + v2x;
            dest[1] = v1y + v2y;
            dest[2] = v1z + v2z;
        }
        ...
        private static float[] edge = new float[3];
        private static float[] diff = new float[3];
        ...
        public static bool IntersectSegmentPoly2D(float[] p0, float[] p1, float[] verts, int nverts, out float tmin, out float tmax, out int segMin, out int segMax)
        {
            ...
            VSubEx(p1[0], p1[1], p1[2], p0[0], p0[1], p0[2], ref dir);

            for (int i = 0, j = nverts-1; i < nverts; j=i++)
            {
                VSubEx(verts[i*3 + 0], verts[i*3 + 1], verts[i*3 + 2], verts[j*3 + 0], verts[j*3 + 1], verts[j*3 + 2], ref edge);
                VSubEx(p0[0], p0[1], p0[2], verts[j * 3 + 0], verts[j * 3 + 1], verts[j * 3 + 2], ref diff);
