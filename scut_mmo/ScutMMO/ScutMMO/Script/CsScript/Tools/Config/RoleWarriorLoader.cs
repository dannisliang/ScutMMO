...
public class RoleWarriorAttributeConfig
{
	public int   Type; // 类型 
	public int   Value; // 值 
}
...
public class RoleWarriorConfig 
{
	public RoleWarriorAttributeConfig[]   attribute; // 属性 
	public int   lv; // 玩家等级 
	public int   upgradePoint; // 升级获得点数 

}

public class RoleWarriorConfigTable 
{
    Dictionary<int, RoleWarriorConfig> m_configs = 
        new Dictionary<int, RoleWarriorConfig>();
    public Dictionary<int, RoleWarriorConfig> configs   
    {
        get {
            return m_configs; 
        }
    ...
    public object ConfigProcess(string[] row)
    {
        ...
        RoleWarriorConfig rec = new RoleWarriorConfig();

		rec.lv = Utility.ToInt(rh.Read());	 //玩家等级
		rec.upgradePoint = Utility.ToInt(rh.Read());	 //升级获得点数
		rec.attribute = new RoleWarriorAttributeConfig[22]; 
		for (int i=0;i<22;i++) {
			rec.attribute[i] = new RoleWarriorAttributeConfig();
		}
    ...
    public void Load()
    {
        ...
        reader.LoadText("Data/Config/Role_warrior.txt", 1);
        int rows = reader.GetRowCount();
        for (int r = 0; r < rows; ++r)
        {
            string[] row = reader.GetRow(r);
            RoleWarriorConfig ac = ConfigProcess(row) as RoleWarriorConfig;
