...
public class RoleShooterAttributeConfig
{
	public int   Type; // 类型 
	public int   Value; // 值 
}
...
public class RoleShooterConfig 
{
	public RoleShooterAttributeConfig[]   attribute; // 属性 
	public int   lv; // 玩家等级 
	public int   upgradePoint; // 升级获得点数 

}

public class RoleShooterConfigTable 
{
    Dictionary<int, RoleShooterConfig> m_configs = 
        new Dictionary<int, RoleShooterConfig>();
    public Dictionary<int, RoleShooterConfig> configs   
    {
        get {
            return m_configs; 
        }
    ...
    public object ConfigProcess(string[] row)
    {
        ...
        RoleShooterConfig rec = new RoleShooterConfig();

		rec.lv = Utility.ToInt(rh.Read());	 //玩家等级
		rec.upgradePoint = Utility.ToInt(rh.Read());	 //升级获得点数
		rec.attribute = new RoleShooterAttributeConfig[22]; 
		for (int i=0;i<22;i++) {
			rec.attribute[i] = new RoleShooterAttributeConfig();
		}
    ...
    public void Load()
    {
        ...
        reader.LoadText("Data/Config/Role_shooter.txt", 1);
        int rows = reader.GetRowCount();
        for (int r = 0; r < rows; ++r)
        {
            string[] row = reader.GetRow(r);
            RoleShooterConfig ac = ConfigProcess(row) as RoleShooterConfig;
